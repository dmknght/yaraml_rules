...
def main():
    ...
    parser.add_argument("benignware_paths", default=None,
                        help="Path to benignware training files, searched recursively")
    parser.add_argument("--max_benign_files", default=None, type=int,
                        help="Sample n benign files without replacement (if not defined use all)")
    parser.add_argument("--max_malicious_files", default=None, type=int,
                        help="Sample n malicious files without replacement (if not defined use all)")
    parser.add_argument("--retrain", action="store_true", default=False,
                        help="Use this if you'd like to retrain a model using an extracted feature pickle file.  "
                             "model_path must point to a directory with saved model parameters.  yaraml will load the "
                             "pickle file in that directory and train a new model with the parameters you specify "
                             "with --model_type or --model_instantion.  You can also change --max_features in your "
                             "retrain.  This is useful in iterating on a yaraml rule until you get to an adequately "
                             "performant model.")
    ...
    parser.add_argument("--ignore_saved_features", default=False, action="store_true",
                        help="Ignore cached features, re-extract features")
    parser.add_argument(
    ...
    parser.add_argument(
        "--model_instantiation",
        help="Use this as an alternative to --model_type, include a line of python instantiating a "
             "RandomForestClassifier or LogisticRegression model "
    )
    parser.add_argument(
        "--max_features",
        default=512,
    ...
    # build and save rule
    if type(classifier) == RandomForestClassifier:
        rule = convert_tree.convert_randomforest(args.rule_name, classifier, vectorizer.get_feature_names(),
                                                 args.detection_threshold)
    else:
        rule = convert_linear.convert_linear(args.rule_name, classifier, vectorizer.get_feature_names(),
                                             args.detection_threshold)
